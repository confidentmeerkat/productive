...
export class UsersService {
  ...
  async create(createUserDto: CreateUserDto & { password_hash: string }): Promise<Omit<User, 'password'>> {
    const { username, email, password_hash } = createUserDto;

    // Check if username or email already exists
    const existingUser = await this.db.query.users.findFirst({
      where: (table, { or }) => or(eq(table.username, username), eq(table.email, email)),
    });
    ...
    const insertedUsers = await this.db.insert(users).values(newUser).returning({
        id: users.id,
